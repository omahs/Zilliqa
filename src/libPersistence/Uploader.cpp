...
#if 0
bool Uploader::IsUploadOngoing() const {
...
std::optional<uint64_t> Uploader::GetCurrentTxBlkNum() const try {
...
void Uploader::DownloadStaticDb() {
  //
  std::error_code errorCode;
...
void Uploader::DownloadPersistenceAndStateDeltas() {
  std::error_code errorCode;
...
void Uploader::DownloadDiffs(uint64_t fromTxBlk, uint64_t toTxBlk,
...
void Uploader::DownloadPersistenceDiff(uint64_t fromTxBlk, uint64_t toTxBlk) {
  std::error_code errorCode;
  std::filesystem::remove(PersistenceDiffPath(), errorCode);
...
void Uploader::DownloadStateDeltaDiff(uint64_t fromTxBlk, uint64_t toTxBlk) {
  std::error_code errorCode;
  std::filesystem::create_directories(StateDeltaPath(), errorCode);
...
Uploader::RetrieveBucketObjects(const std::string& prefix,
                                  bool excludePersistenceDiff /* = true*/) {
...
Uploader::DownloadFutures Uploader::DownloadBucketObjects(
